...
use AppBundle\Service\ExchangeClient as ExchangeClient;
...
Class CryptopiaClient extends ExchangeClient
{
	...
	private function send($method = null, $args = array(), $secure = true) {
		if (empty($method)) {
			return $this->getErrorReturn("method was not defined!");
		}
		$urlParams = $args;
		...
		if ($obj = json_decode($execResult, true)) {
			if ($obj["Success"] == true) {
				if (!isSet($obj["Error"])) {
					return $this->getReturn($obj["Success"], $obj["Error"], $obj["Data"]);
				} else {
	...
	public function getCurrencyUrl($args = null) {
		...
		if (!isSet($args["market"])) {
			return $this->getErrorReturn("required parameter: market");
		}
	...
	public function getOrder($args = null) {
		if (!isSet($args["orderid"])) {
			return $this->getErrorReturn("required parameter: orderid");
		}
	...
	public function buy($args = null) {
		...
		if (!isSet($args["rate"])) {
			return $this->getErrorReturn("required parameter: rate");
		}
		$args["Rate"] = $args["rate"];
		unset($args["rate"]);
		if (!isSet($args["amount"])) {
			return $this->getErrorReturn("required parameter: amount");
		}
